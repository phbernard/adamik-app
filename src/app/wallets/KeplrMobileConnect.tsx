...
export const KeplrMobileConnect: React.FC<KeplrConnectProps> = ({
  setWalletAddresses,
}) => {
  const { status, client } = useWalletClient("keplr-mobile");
  ...
  const connect = async () => {
    try {
      ...
      toast({
        description:
          "Connected to Keplr Mobile, please check portfolio page to see your assets",
      });
    } catch (err) {
